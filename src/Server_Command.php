...
class Server_Command extends FP_CLI_Command {
	...
	public function __invoke( $_, $assoc_args ) {
		$defaults   = array(
			'host'    => 'localhost',
			'port'    => 8080,
			'docroot' => ! is_null( FP_CLI::get_runner()->config['path'] ) ? FP_CLI::get_runner()->config['path'] : false,
		...
		if ( ! $docroot ) {
			$config_path = FP_CLI::get_runner()->project_config_path;
		...
		if ( ! file_exists( $router_path ) ) {
			FP_CLI::error( "Couldn't find router.php" );
		}
		$cmd = Utils\esc_cmd(
			'%s -S %s -t %s -c %s %s',
			FP_CLI::get_php_binary(),
